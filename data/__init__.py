...
def create_dataloader(dataset, dataset_config, phase):
    '''create dataloader '''
    if phase == 'train':
        ...
        if batch_size > 1:
            sampler = SequentialBinSampler(dataset.get_file_lengths())
            return torch.utils.data.DataLoader(
                dataset,
                batch_size=batch_size,
                shuffle=False,
                num_workers=dataset_config.val_num_workers,
                pin_memory=True,
                sampler=sampler,
...
def create_dataset(dataset_config, phase):
    ...
    dataset = AudioDataset(json_dir=json_dir,
